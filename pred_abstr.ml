...
(*Debug.print_debug ("Looking at invariant " ^ (string_of_invariant (heap1,heap2,preds,inv_preds,env)) ^ " and " ^  (string_of_invariant (heap1',heap2',preds',inv_preds',env')) ^ ".");*)
...
let generate_transition pred_env pred_vals ((_,pred_val1) as ps1) = function
  ...
  | OT (s2,polarity) ->[((s2,pred_val1),polarity)]
(*    let ps2_list = generate_pstate_list s2 pred_vals in
    List.map (fun ps2 -> (ps2,polarity)) ps2_list*)
  | OET s2 -> [((s2,pred_val1),OE)]
(*    let ps2_list = generate_pstate_list s2 pred_vals in
    List.map (fun ps2 -> (ps2,OE)) ps2_list*)
