...
let rec typing vctx lctx expr = match expr with
  ...
  | If (e1,e2,e3) ->
    ...
    Debug.print_debug ("Typing of the first branch:" ^ (string_of_typeML ty2));
    Debug.print_debug ("Typing of the second branch:" ^ (string_of_typeML ty3));
    begin match unify_type Pmap.empty (ty2,ty3) with
      | Some (ty,lsubst) -> Debug.print_debug ("Typing If:" ^ (string_of_typeML ty)); (ty,lsubst_vctx lsubst vctx3)
  ...
  | App (e1,e2) ->
    ...
    begin match fty with
      ...
      | (TArrow (ty',ty),_) when ty' <> aty ->
        Error.fail_error ("Error typing " ^ (Syntax.string_of_exprML expr) ^ ": "
                  ^ (string_of_typeML ty') ^ " is not equal to "
                  ^(string_of_typeML aty)) *)
      | _ -> Error.fail_error ("Error typing " ^(Syntax.string_of_exprML expr) ^ ": "
