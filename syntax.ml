...
let rec unify_type lsubst = function
  ...
  | (TArrow (ty11,ty12), TArrow (ty21,ty22)) ->
    begin match unify_type lsubst (ty11,ty21) with
      | None -> Debug.print_debug ("Cannot unify " ^ (string_of_typeML ty11) ^ " and " ^ (string_of_typeML ty21)); None
      | Some (ty1,lsubst') ->
        begin match unify_type lsubst' (ty12,ty22) with
          | None -> Debug.print_debug ("Cannot unify " ^ (string_of_typeML ty12) ^ " and " ^ (string_of_typeML ty22)); None
    ...
    begin match lookup_pmap tvar lsubst with
      ...
      | Some ty' ->
        begin match unify_type lsubst (ty,ty') with
          | None -> Debug.print_debug ("Cannot unify " ^ (string_of_typeML ty) ^ " and " ^ (string_of_typeML ty')); None
          | Some (ty'',lsubst'') -> Some (ty'', (tvar,ty'')::lsubst'')
        end
    end
  | (ty1,ty2) -> Debug.print_debug ("Cannot unify " ^ (string_of_typeML ty1) ^ " and " ^ (string_of_typeML ty2)); None
...
let rec subst expr value value' = match expr with
  ...
  | LetPair (var1,var2,expr1,expr2) when
    ((Var var1 <> value) && (Var var2 <> value)) ->
        LetPair (var1,var2, subst expr1 value value', subst expr2 value value')
  | LetPair (var1,var2,expr1,expr2) -> LetPair (var1,var2, subst expr1 value value', expr2)
...
let rec string_of_exprML = function
  ...
  | LetPair (var1,var2,e1,e2) -> "let (" ^ var1 ^ "," ^ var2 ^ ") = " ^ (string_of_exprML e1) ^ " in " ^ (string_of_exprML e2)
