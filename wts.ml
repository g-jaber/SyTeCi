...
let string_of_label (heapPre1,heapPre2,heapPost1,heapPost2,preds,_,polarity) =
  let string_heaps = (string_of_symb_heap heapPre1) ^ "↝"
...
let print_sr sr =
  ...
  let aux (s1,l) = List.iter (fun trans -> print_endline (string_of_transition s1 trans)) l in
  List.iter aux (StateMap.bindings sr.trans_fun)
...
let union ((sr1,isExt1,isWB1,preds1,p_back_trans1) as esr1)
    ((sr2,isExt2,isWB2,preds2,p_back_trans2) as esr2) =
  ...
  | (false,true) ->
    failwith ("Error trying to build the WTS: We are trying to merge two ESRs
               which already share the same initial state")
...
let build_esr_ruleO polarity = function
  | [] ->
    failwith "Error trying to build the WTS: the rule Init, V or K does not contain any premise"
  | esr::esrs ->
    let new_init_state = fresh_state () in
    let (sr,isExt,isWB,preds,p_back_trans) =
      List.fold_left (basic_union new_init_state) esr esrs in
    let wb_transitions =
      List.map (fun state' -> (new_init_state, state')) (States.elements isWB) in
    let extern_transitions =
      List.map (fun state' -> (new_init_state, state')) (States.elements isExt) in
    let o_transitions =
      List.map (fun (sr,_,_,_,_) -> OT (sr.init_state,polarity)) (esr::esrs) in
    ...
    Debug.print_debug (string_of_opolarity polarity);
    begin match polarity with
      | OI | OQ -> (sr,States.empty,States.empty,preds,p_back_trans)
      | OA -> (sr,States.empty,isWB,preds,p_back_trans)
    end

let add_incons_trans (hpre1,hpre2,hpost1,hpost2,preds,preds',var_ctx,polarity)
    ((sr,_,_,_,_) as esr) =
  ...
  let incons_trans = PT (s',(hpre1,hpre2,hpost1,hpost2,full_neg_preds,var_ctx
                            ,polarity)) in
...
let build_pintern_trans sequent ((sr1,_,_,_,_) as esr1)
    ((sr2,_,_,preds,_) as esr2,(tag,hpre1,hpre2,hpost1,hpost2),sequent') =
  ...
  match (Logic_to_z3.check_sat sequent'.ground_var_ctx [full_preds],
    ...
    | (false,false) -> esr1
    | (false,true) -> add_incons_trans (hpre1,hpre2,hpost1,hpost2,preds,preds'
                                       ,sequent'.ground_var_ctx,polarity) esr1
    | (_,b) ->
      let trans = PT (sr2.init_state,(hpre1,hpre2,hpost1,hpost2,full_preds
                                     ,sequent'.ground_var_ctx,polarity)) in
      let (sr,isExt,isWB,_,back_trans) = basic_union sr1.init_state esr1 esr2 in
      let isExt' = if (tag = Extern) then States.add (sr2.init_state) isExt
        else isExt in
      let isWB' = if (tag = WB) then States.add (sr2.init_state) isWB
        else isWB in
...
let rec build_esr bc = function
  | RuleVG sequent ->
    let sr = singleton_sr () in
    (sr,States.empty,States.empty,[extract_pred_from_vg sequent],[])
  | Stop _ | ROut _ | RuleSwrong _  ->
    let sr = singleton_sr () in
    (sr,States.empty,States.empty,[AFalse],[])
  | LOut _   ->
    let sr = singleton_sr () in
  ...
  | Unfold (tc,_) | LUnfold (tc,_) | RUnfold (tc,_)  | Rewrite (tc,_) ->
    build_esr bc tc
  | RuleInit (tcs,_) ->
    let esrs = List.map (build_esr bc) tcs in
    build_esr_ruleO OI esrs
  | RuleV (tcs,_) ->
    let esrs = List.map (build_esr bc) tcs in
    build_esr_ruleO OQ esrs
  | RuleK (tcs,_) ->
    let esrs = List.map (build_esr bc) tcs in
    build_esr_ruleO OA esrs
  | RuleE (tcs_a,sequent) ->
    let aux (annotation,tc) = (build_esr bc tc,annotation,get_root tc) in
    let esrs_a = List.map aux tcs_a in
...
let build_sr bc tc =
  ...
  | (sr,_,_,_,[]) -> sr
  | (_,_,_,_,_) ->
    failwith ("Error, the generation of back-transition is not finished. ");
