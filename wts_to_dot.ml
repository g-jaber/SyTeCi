...
let dlabel_from_astate sr s =
  let n = string_of_int (atom_state_from_state s) in
  ...
  n ^ "[shape = "^ shape ^", label=\""^ l ^ "\"];"


let degde_from_pintern_transition (s1,s2,label) = 
  let n1 = string_of_int (atom_state_from_state s1) in
  let n2 = string_of_int (atom_state_from_state s2) in
...
let dot_from_sr sr =
  ...
  States.iter (fun x -> print_endline ((dlabel_from_astate sr x) ^ " ")) sr.states;
  print_newline ();
